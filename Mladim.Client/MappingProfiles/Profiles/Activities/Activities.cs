...
public class Activities : Profile
{
    public Activities()
    {
        CreateMap<ActivityVM, UpdateActivityCommandDto>()
             .ForMember(dest => dest.TimeRange, m => m.MapFrom(src => DateTimeRangeCommandDto.Create(src.DateRange.Start.Value.ToUniversalTime(), src.DateRange.End.Value.ToUniversalTime(), src.StartTime, src.EndTime)))
